...
{
	App->render->AddBlitEvent(6, minimap_camera, 120 + position.x - App->render->camera.x/App->win->scale - (App->render->camera.x / 50) / App->win->scale, position.y - App->render->camera.y / App->win->scale - (App->render->camera.y / 50) / App->win->scale, cameraminimap, false);
}
